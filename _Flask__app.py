...
engine = get_mysql_engine('mino', 'XX', 'XX', 'Mino')

persist_directory = '/var/www/flaskapp/files'
...
def chat():
    ...
    response_message = chat_with_mino(user_message, history=[])
    response_message_html = markdown.markdown(response_message)
    return jsonify ({'response' : response_message_html})
